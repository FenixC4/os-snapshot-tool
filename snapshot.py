...
def producer_task(task_queue, directories, exclusion_set):
    ...
    for root_dir in directories:
        if not os.path.exists(root_dir):
            print(
                f"[Producer] Warning: Scan path '{root_dir}' does not exist. Skipping.",
                file=sys.stderr,
            )
            continue
        if not os.access(root_dir, os.R_OK):
            print(
                f"[Producer] Warning: No read access to {root_dir}. Skipping.",
                file=sys.stderr,
            )
            continue
        try:
...
def writer_task(results_queue, db_file, table_name, worker_count):
    ...
    while workers_finished < worker_count:
        ...
        if len(batch) >= 2000:
            cursor.executemany(
                f"INSERT OR REPLACE INTO {table_name} VALUES (?, ?, ?, ?, ?, ?)", batch
            )
            conn.commit()
...
def load_before_cache(db_file):
    ...
    try:
        cursor.execute(
            "SELECT full_path, size_bytes, mod_time, file_hash FROM snapshot_before"
        )
...
def run_snapshot(args):
    ...
    if args.command == "after" and not before_cache:
        print(
            "Error: 'before' snapshot not found. Please run the 'before' command first.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    with Manager() as manager:
        ...
        writer_proc = Process(
            target=writer_task,
            args=(results_queue, args.db_file, table_name, args.workers),
        )
        writer_proc.start()

        pool_args = (task_queue, results_queue, before_cache, PlatformAdapter())
        with Pool(
            processes=args.workers, initializer=consumer_worker, initargs=pool_args
        ) as pool:
...
def run_analysis(args):
    ...
    try:
        cursor.execute(
            "SELECT full_path, size_bytes FROM snapshot_after WHERE full_path NOT IN (SELECT full_path FROM snapshot_before)"
        )
        ...
        cursor.execute(
            "SELECT full_path FROM snapshot_before WHERE full_path NOT IN (SELECT full_path FROM snapshot_after)"
        )
        ...
        for (path,) in deleted_files:
            print(f"  - {path}")
        cursor.execute(
            "SELECT a.full_path, a.size_bytes FROM snapshot_after a JOIN snapshot_before b ON a.full_path = b.full_path WHERE a.file_hash != b.file_hash"
        )
        modified_files = cursor.fetchall()
        print(f"\n[*] {len(modified_files)} Files Modified:")
        for path, size in modified_files:
            print(f"  - {path} (new size: {size} bytes)")
    except sqlite3.OperationalError as e:
        print(
            f"\nError during analysis: {e}. Did you run both 'before' and 'after' snapshots?",
            file=sys.stderr,
        )
    finally:
        conn.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A high-performance filesystem snapshot and analysis tool."
    )
    ...
    parent_parser.add_argument(
        "-f",
        "--db-file",
        default="filesystem_snapshot.db",
        help="Path to the SQLite database file (default: filesystem_snapshot.db)",
    )
    parent_parser.add_argument(
        "-s",
        "--scan",
        action="append",
        default=platform_adapter.default_scan_dirs,
        help="Directory to scan (can be specified multiple times). Defaults to OS-specific paths.",
    )
    parent_parser.add_argument(
        "-e",
        "--exclude",
        action="append",
        default=platform_adapter.default_exclude_dirs,
        help="Directory to exclude from scan (can be specified multiple times). Defaults to OS-specific paths.",
    )
    parent_parser.add_argument(
        "-w",
        "--workers",
    ...
    # --- 'before' command ---
    parser_before = subparsers.add_parser(
        "before",
        help="Create the initial 'before' snapshot of the filesystem.",
        parents=[parent_parser],
    )
    parser_before.set_defaults(func=run_snapshot)

    # --- 'after' command ---
    parser_after = subparsers.add_parser(
        "after",
        help="Create the 'after' snapshot to compare against the 'before' state.",
        parents=[parent_parser],
    )
    ...
    parser_analyze = subparsers.add_parser(
        "analyze",
        help="Analyze the differences between the 'before' and 'after' snapshots.",
    )
    parser_analyze.add_argument(
        "-f",
        "--db-file",
        default="filesystem_snapshot.db",
        help="Path to the SQLite database file to analyze (default: filesystem_snapshot.db)",
    )
